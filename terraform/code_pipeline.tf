...
####################################################
locals {
  aws_ecr_codebuild_image_url                       = "412335340103.dkr.ecr.ap-northeast-1.amazonaws.com/tfpractice-codebuild"
  IAM_POLICY_ARN_KMSReadOnly                        = "arn:aws:iam::412335340103:policy/tfpractice-kms-readonly"
  ...
  policy = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:codebuild:ap-northeast-1:412335340103:report-group/*"
      ]
    }
  ]
}
...
resource "aws_iam_policy" "build_secret" {
  policy = file("./json/code_build/CodeBuildManagedSecretPolicy.json")
}
  ...
  policy = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:ec2:ap-northeast-1:412335340103:network-interface/*",
      "Condition": {
        "StringEquals": {
          "ec2:Subnet": [
